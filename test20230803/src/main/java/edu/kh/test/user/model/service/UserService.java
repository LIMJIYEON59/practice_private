...
public class UserService {
	...
	public UserDTO selectOne(int userNo) {
		...
		// 1.데이터베이스 연결을 얻어옴	//jdbctemplate부터 geconnection을 호출한다.
		Connection conn = JdbcTemplate.getConnection();	
		
		// dao의 selectOne 메서드를 호출하여 회원 정보 조회	//dao를 호출하고 그 값을 result에 담는다.	//conn을 가지고 dao로 간다.(연결통로를 줄거고 view로 부터 들어 온 userNo도 주겠다.) 
		result = dao.selectOne(conn, userNo); 
		
		//데이터베이스 연결을 닫음 (위에 호출하고 나서 끊어줌)
