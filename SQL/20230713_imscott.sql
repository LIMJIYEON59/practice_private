...
--<서브쿼리> 242P
--:두 개의 SELECT문을 하나의 SELECT문으로 합쳐서 사용가능
-- 서브쿼리로 JONES의 급여보다 높은 급여를 받는 사원 정보 출력하기 
    --:기준 급여보다 높은 급여를 받는 사원을 메인쿼리로 작성하고 기준급여(JONES의 급여)가 서브쿼리가 된다.
SELECT *
FROM EMP
...
--서브쿼리를 사용하여 EMP 테이블의 사원 정보 중에서 사원 이름이 ALLEN인 사원의 추가 수당보다 많은 추가 수당을 받는 사원 정보를 구하도록 다음 코드를 채워 보세요
SELECT *
FROM EMP
...
--실행 결과가 하나인 단일행 서브쿼리
    --:서브쿼리에서 출력되는 결과가 하나이다. 단일행 연산자를 사용하여 비교한다. ( <>, ^=, != 같지않음)
...
create user LJY identified by LJY;
grant connect, resource, dba to LJY;
grant create table to LJY;
